 ...
 * @returns
 */
export function VuiPlusAlias(): Plugin {
  const themeChalk = 'theme-chalk'
  const sourceThemeChalk = `${PKG_PREFIX}/${themeChalk}` as const
  const bundleThemeChalk = `${PKG_NAME}/${themeChalk}` as const

  return {
    name: 'v-ui-plus-alias-plugin',
    resolveId(id) {
 ...
 * @returns
 */
export function VuiPlusExternal(options: OutputOptions): Plugin {
  ...
  return {
    name: 'v-ui-plus-external-plugin',
    resolveId(id) {
 ...
 * @returns
 */
export function VuiPlusClearConsole(): Plugin {
  const reg = /(console.log()(.*)())/g
  return {
    name: 'v-ui-plus-clear-console-plugin',
    transform(source) {
